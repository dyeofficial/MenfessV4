...
const isOwner = [`${setting.ownerNumber}`,"6283142259306@s.whatsapp.net","6283142259306@s.whatsapp.net"].includes(sender) ? true : false
...
const gurbot = '6283142259306@s.whatsapp.net'
...
let btn_menu = [
{buttonId: `${prefix}credit`, buttonText: { displayText: '⋮☰ 𝗖𝗥𝗘𝗗𝗜𝗧' }, type: 1 },
...
var teks2 = `\n\nSukses mengirim ke owner`
var bg_lexxy = '6283142259306@s.whatsapp.net'
...
conn.sendMessage(from, { caption: cap, image: { url: feta.data.qris }, buttons: buto, footer: '© created by Dÿè ' })
}
console.log(feta)
}
break
...
.then(data => {
var opt = { packname: 'Dÿè - Bot  么', author: 'By Dÿè' }
...
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
const cct =['60','61','62','63','64','65','66','67','68','69','70','71','72','73','74','75','76','77','78','79','80','81','82','83','84','85','86','87','88','89','90','91','92','93','94','95','96','97','98','99','100']
const oll = cct[Math.floor(Math.random() * cct.length)]
conn.sendMessage(from, { text: `${oll}%` }, { quoted: msg })
break
...
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
const eyy =['60','61','62','63','64','65','66','67','68','69','70','71','72','73','74','75','76','77','78','79','80','81','82','83','84','85','86','87','88','89','90','91','92','93','94','95','96','97','98','99','100']
...
if (!q) return reply(`Syntak Error!\n*Contoh:*\n${prefix+command} 628xxx`)
var num = q+"@s.whatsapp.net"
var dev = '6283142259306@s.whatsapp.net'
if (num == dev) return reply('Itu owner gua')
if (num == sender) return reply('itu nomor lu sendiri')
...
break
case 'bot':
reply(`Ada Apa Kak? Dÿè Bot Aktif Kok😊`) 
break
case 'dyebot':
...
case 'truth':
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
const teruth =['Pernah suka sama siapa aja? berapa lama?','Kalau boleh atau kalau mau, di gc/luar gc siapa yang akan kamu jadikan sahabat?(boleh beda/sma jenis)','apa ketakutan terbesar kamu?','pernah suka sama orang dan merasa orang itu suka sama kamu juga?','Siapa nama mantan pacar teman mu yang pernah kamu sukai diam diam?','hal yang bikin seneng pas lu lagi sedih apa','pernah cinta bertepuk sebelah tangan? kalo pernah sama siapa? rasanya gimana brou?','hal yang paling ditakutin','siapa orang yang paling berpengaruh kepada kehidupanmu','hal membanggakan apa yang kamu dapatkan di tahun ini','(bgi yg muslim) pernah ga solat seharian?','Siapa yang paling mendekati tipe pasangan idealmu di sini','suka mabar(main bareng)sama siapa?','pernah nolak orang? alasannya kenapa?','Sebutkan kejadian yang bikin kamu sakit hati yang masih di inget','pencapaian yang udah didapet apa aja ditahun ini?','kebiasaan terburuk lo pas di sekolah apa?','sebutkan inisial crush mu','apa hal yang paling memalukan yang pernah kamu lakuin?','masih sayang mantan?','pernah kehilangan sahabat?','pernah ga sih mimpiin crush lo?','siapa teman terbaikmu','siapa cinta pertamamu','siapa digrup ini yang paling dekat denganmu','siapa menurut mu kamu merasa canggung jika harus berdua?','apa cita-citamu?','hal apa yang membuat mu tidak pede?','apa film favoritmu?','apa makanan favorit mu?','apa makanan yang tidak kamu suka?','apa mimpi teraneh yang pernag kamu alami?']
const ttr = teruth[Math.floor(Math.random() * teruth.length)]
conn.sendMessage(from, { text: `*──「 TRUTH 」──*\n *Nama: @${sender.split('@')[0]}*\n *Perintah: #truth*\n *Pertanyaan:*\n\n⤖ ${ttr}` }, { quoted: msg })
break

case 'dare':
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
const deer =['Kirim pesan ke mantan kamu dan bilang "aku masih suka sama kamu','telfon crush/pacar sekarang dan ss ke pemain','pap ke salah satu anggota grup','Bilang "KAMU CANTIK BANGET NGGAK BOHONG" ke cowo','ss recent call whatsapp','drop emot "😎??" setiap ngetik di gc/pc selama 1 jam','chat ke kontak wa urutan sesuai %batre kamu, terus bilang ke dia "i lucky to hv you','prank chat mantan dan bilang " i love u, pgn balikan','record voice baca surah al-kautsar','bilang "i hv crush on you, mau jadi pacarku gak?" ke lawan jenis yang terakhir bgt kamu chat (serah di wa/tele), tunggu dia bales, kalo udah ss drop ke sini','sebutkan tipe pacar mu!','snap/post foto pacar/crush','teriak gajelas lalu kirim pake vn kesini','pap mukamu lalu kirim ke salah satu temanmu','kirim fotomu dengan caption, aku anak pungut','Pura pura kerasukan, contoh : kerasukan maung, kerasukan belalang, kerasukan kulkas, dll','tunjukkan history pencarian googlemu','Telpon @3']
const ddr = deer[Math.floor(Math.random() * deer.length)]
conn.sendMessage(from, { text: `*──「 DARE 」──*\n *Nama: @${sender.split('@')[0]}*\n *Perintah: #dare*\n *Tantangan:*\n\n⤖ ${ddr}` }, { quoted: msg })
break

///stay halal

case 'xnxx': case 'xxx': case 'xvidoes': case 'pornhub': case 'ph': case 'hentai': case 'xhentai': case 'bokeb': case 'bkpindo': case 'xvid':
reply(`Astaghfirullah tobatlah kawan sebelum telat bukankah Allah telah melarang kita untuk mendekati zina? \n\nAllah swt Berfirman, yang artinya: “Katakanlah kepada orang laki-laki yang beriman: Hendaklah mereka menahan pandanganya, dan memelihara kemaluannya; … Katakanlah kepada wanita yang beriman: Hendaklah mereka menahan pandangannya, dan kemaluannya, dan janganlah mereka Menampakkan perhiasannya, kecuali yang (biasa) nampak dari padany,” (TQS. Al-Nur [24]: 30-31).\n\n Imam Ibn Katsir dalam kitab tafsirnya menjelaskan: “Ini adalah perintah dari Allah swt kepada hamba-hamba-Nya yang beriman agar menundukkan pandangan mereka dari apa-apa yang diharamkan atas mereka.” Tidak ada perbedaan dalam hal ini bahwa yang diharamkan untuk dipandang adalah aurat. Berdasarkan riwayat dari Bahz bin Hakim, dari ayahnya, dari kakeknya berkata: “Wahai Rasulullah saw, terhadap aurat-aurat kami, apa yang boleh kami lakukan dan apa yang harus kami hindari? Beliau saw berkata:’Jagalah auratmu kecuali atas istri dan budak perempuanmu.’” (THR. Ahmad bin Hanbal)[2]`) 
break

///offf

case 'credit':
reply(`*#CREDIT*\n\n — 𝙎𝙥𝙚𝙘𝙞𝙖𝙡 𝙏𝙝𝙖𝙣𝙠𝙨 𝙁𝙤𝙧 𝙈𝙮 𝙁𝙧𝙞𝙚𝙣𝙙𝙨 — \n\n » 𝙽𝚊𝚘 \n » 𝙲𝚑𝚒𝚣𝚊𝚛𝚞 \n » 𝙰𝚛𝚐𝚊𝚜 𝙲𝙷 \n » 𝙽𝚒𝚜𝚊 𝚃𝚊𝚖𝚟𝚊𝚗 \n » 𝙳𝚢𝚎 ( 𝚜𝚊𝚢𝚊 𝚜𝚎𝚗𝚍𝚒𝚛𝚒 ) \n\n Terimakasih banyak untuk teman-teman saya yang sudah membantu perkembangan bot ini ^_^ \n 𝙰𝚛𝚒𝚐𝚊𝚝𝚘𝚞...`) 
break
